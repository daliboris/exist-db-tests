<xsl:stylesheet
 ...
 <!-- 
  TODO: 
        - run "ant secrets.prepare" to create the local.secrets.xsl file if it does not exist
        - edit the local.secrets.xsl file to add your users and groups (see template "permissions" below)
        - run "ant build" to create the .xar file with the users and groups defined in local.secrets.xsl
        - crate local.secrets.xsl file (see <xsl:import />)
